...
{
   float deltaX = 1.0 / numPixelsOnWidth;
   float deltaY = 1.0 / numPixelsOnWidth;
   float deltaA = 1.0 / pow(numPixelsOnWidth / 2.0, 2.0);

   for ( float i = 0.0; i < numPixelsOnWidth; i++) {
       for ( float j = 0.0; j < numPixelsOnWidth; j++) {
           float x = -1.0 + deltaX + (i * (2.0 / numPixelsOnWidth));
           float y = -1.0 + deltaY + (j * (2.0 / numPixelsOnWidth));

           float r = pow((pow(x, 2.0) + pow(y, 2.0) + 1.0), 0.5);
           float cos_phi_i = 1 / pow((pow(x, 2.0) + pow(y, 2.0) + 1.0), 0.5);
           float cos_phi_j = cos_phi_i;

           float form_factor_numerator = deltaA * cos_phi_i * cos_phi_j;
           float form_factor_denominator = (M_PI * pow(r, 2.0));

           deltaFormFactors[numPixelsOnWidth * (int)i + (int)j] = form_factor_numerator / form_factor_denominator;
       }
   }
}
...
{
   float deltaY = 1.0 / numPixelsOnWidth;
   float deltaZ = 1.0 / numPixelsOnWidth;
   float deltaA = 1.0 / pow(numPixelsOnWidth / 2.0, 2.0);

    for ( float i = 0.0; i < numPixelsOnWidth / 2; i++) {
        for ( float j = 0.0; j < numPixelsOnWidth; j++) {
            float z = deltaZ + (i * (2.0 / numPixelsOnWidth));
            float y = -1.0 + deltaY + (j * (2.0 / numPixelsOnWidth));

            float form_factor_numerator = deltaA * z;
            float form_factor_denominator = M_PI * pow((pow(y, 2.0) + pow(z, 2.0) + 1.0), 2.0);
    ...
    // Need to set up the viewport, projection and view transfromation.
{
    ...
    float lookUpVec[3]; //glLookAt up vec

    float helperVec[3];
    VecDiff(helperVec, shooterQuad->v[0], shooterQuad->v[1]);
    
    VecCrossProd(lookUpVec, shooterQuad->normal, helperVec);

    float lookEyeVec[3];
    for ( int i = 0; i < 3; i++ ){
        lookEyeVec[i] = shooterQuad->centroid[i];
    }

    float lookCenterVec[3];
    for ( int i = 0; i < 3; i++ ){
        lookCenterVec[i] = shooterQuad->centroid[i] + shooterQuad->normal[i];
    }

    gluLookAt(
        lookEyeVec[0], lookEyeVec[1], lookEyeVec[2],
        lookCenterVec[0], lookCenterVec[1], lookCenterVec[2],
        lookUpVec[0], lookUpVec[1], lookUpVec[2]
    );
}
    ...
    // Need to set up the viewport, projection and view transfromation.
{
   ...
   glFrustum(-nearPlane, nearPlane,
    ...
    if (face == 1) {
        for ( int i = 0; i < 3; i++ ){
            lookVec[i] = helperVec[i];            
        }
    } else if (face == 2) {
        VecCrossProd(lookVec, shooterQuad->normal, helperVec);
    } else if (face == 3) {
        VecNeg(lookVec, helperVec);
    } else if (face == 4) {
        VecNeg(lookVec, VecCrossProd(lookVec, shooterQuad->normal, helperVec));
    }
    ...
    float lookCenterVec[3];
    for ( int i = 0; i < 3; i++ ){
        lookCenterVec[i] = shooterQuad->centroid[i] + lookVec[i];
    }

    float lookUpVec[3];
    for ( int i = 0; i < 3; i++ ){
        lookUpVec[i] = shooterQuad->normal[i];
    }
    ...
    // and update the unshot power of their parent shooter quads.
{
    for ( int i = 0; i < width * height; i++ )
    {
       ...
       for(int j = 0; j < 3; j++){
           m->gatherers[g]->radiosity[j] += m->gatherers[g]->surface->reflectivity[j]*deltaFormFactors[i]*shotPower[j]/m->gatherers[g]->area;
       }

       for(int k = 0; k < 3; k++){
           m->gatherers[g]->shooter->unshotPower[k] += m->gatherers[g]->surface->reflectivity[k] * deltaFormFactors[i] * shotPower[k];
       }
    }
}
