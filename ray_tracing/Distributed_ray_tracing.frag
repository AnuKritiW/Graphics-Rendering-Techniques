                      ...
                      out float t, out vec3 hitPos, out vec3 hitNormal )
{
    ...
    vec3 translated_o = ray.o - sph.center;
    float b = 2.0 * dot(ray.d, translated_o);
    float c = dot(translated_o, translated_o) - pow(sph.radius, 2.0);
    ...
    hitPos = ray.o + t * ray.d;
    hitNormal = (hitPos - sph.center);
    hitNormal = hitNormal / length(hitNormal);
              ...
              out vec3 hitNormal, out vec3 k_rg )
{
    ...
    for (surfaceIdx = 0; surfaceIdx < NUM_PLANES; surfaceIdx++) {
        temp_hasHit = IntersectPlane(Plane[surfaceIdx], ray, DEFAULT_TMIN, DEFAULT_TMAX, temp_t, temp_hitPos, temp_hitNormal);
        ...
        if (temp_hasHit && temp_t < nearest_t) {
            ...
            nearest_hitMatID = Plane[surfaceIdx].materialID;
        }
    }

    for (surfaceIdx = 0; surfaceIdx < NUM_SPHERES; surfaceIdx++) {
        temp_hasHit = IntersectSphere(Sphere[surfaceIdx], ray, DEFAULT_TMIN, DEFAULT_TMAX, temp_t, temp_hitPos, temp_hitNormal);
        ...
        if (temp_hasHit && temp_t < nearest_t) {
            ...
            nearest_hitPos = temp_hitPos;
            nearest_hitNormal = temp_hitNormal;
            nearest_hitMatID = Sphere[surfaceIdx].materialID;
        }
    }
    ...
    for (int i = 0; i < NUM_LIGHTS; i++) {
        Light_t currLight = Light[i];
        vec3 shadowRayDir = currLight.position - nearest_hitPos;
        Ray_t shadowRay;
        shadowRay.o = nearest_hitPos;
        shadowRay.d = normalize(shadowRayDir);
        nearest_hitNormal = normalize(nearest_hitNormal);
        ...
        for(surfaceIdx = 0; surfaceIdx < NUM_PLANES; surfaceIdx++) {
            ...
            if (!temp_hasHit) temp_hasHit = IntersectPlane(Plane[surfaceIdx], shadowRay, DEFAULT_TMIN, length(shadowRayDir));
        }

        for(surfaceIdx = 0; surfaceIdx < NUM_SPHERES; surfaceIdx++) {
            if (!temp_hasHit) temp_hasHit = IntersectSphere(Sphere[surfaceIdx], shadowRay, DEFAULT_TMIN, length(shadowRayDir));
        }

        I_local += PhongLighting(shadowRay.d, nearest_hitNormal, -ray.d, temp_hasHit, Material[nearest_hitMatID], currLight);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 compounded_k_rg;

    float sign_x, sign_y;
    Ray_t nextRay;

    for (float i = 0.0; i < SPP; i++) {
        
        sign_x = (rand() > 0.5) ? 1.0 : -1.0;
        sign_y = (rand() > 0.5) ? 1.0 : -1.0;
        vec3 lens_offset = vec3(sign_x * rand() * aperture_width/2.0, sign_y * rand() * aperture_width/2.0, 0.0);
        lens_offset = lens_offset.x * cam_x_axis  + lens_offset.y * cam_y_axis + lens_offset.z * cam_z_axis;
        vec3 random_pos = cam_pos + lens_offset;

        sign_x = (rand() > 0.5) ? 1.0 : -1.0;
        sign_y = (rand() > 0.5) ? 1.0 : -1.0;    
        // Current pixel 3D position in camera space.
        vec3 pixel_pos = image_origin +  vec3(pixel_width * fragCoord, 0) +
                            vec3(sign_x * rand() * pixel_width/2.0, sign_y * rand() * pixel_width/2.0, 0.0);
        ...
        // pRay.o = aperture_pt;
        pRay.o = random_pos;
        pRay.d = normalize( pixel_pos.x * cam_x_axis  +
                                  pixel_pos.y * cam_y_axis  +
                                  pixel_pos.z * cam_z_axis - lens_offset);
    ...
    I_result = vec3(I_result.x/SPP, I_result.y/SPP, I_result.z/SPP);
