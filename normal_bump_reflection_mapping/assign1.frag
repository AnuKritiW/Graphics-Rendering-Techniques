...
void drawBrickCube()
{
    if (gl_FrontFacing) {
        ...
        vec3 tanPerturbedNormal = normalize(texNormal * 2.0 - 1.0);
        tanPerturbedNormal.z = tanPerturbedNormal.z * DeltaNormal_Z_Scale;
        tanPerturbedNormal = normalize(tanPerturbedNormal);

        vec3 ecPerturbedNormal = tanPerturbedNormal.x * T1 + tanPerturbedNormal.y * B1 + tanPerturbedNormal.z * necNormal;
        ...
        vec3 mixedColor = LightAmbient.rgb * texColor +
                     LightSpecular.rgb * spec + 
                     LightDiffuse.rgb * texColor * N_dot_L;
...
void drawWoodenCube()
{
    if (gl_FrontFacing) {
        ...
        if (sqrDist >= pow(MirrorRadius, 2)) {
            ...
            vec3 texColor = texture(WoodDiffuseMap, texCoords).rgb;     
            ...
            FragColor = vec4(LightAmbient.rgb * texColor + LightDiffuse.rgb * texColor * N_dot_L + LightSpecular.rgb * pf, 1.0);
            ...
            vec3 tanPerturbedNormal = normalize( vec3(p.x, p.y, sqrt(MirrorRadius * MirrorRadius - p.y * p.y)));
        ...
        // FragColor = vec4(0.0, 0.0, 0.0, 1.0);  // Replace this with your code.
    }
    else discard;
}
